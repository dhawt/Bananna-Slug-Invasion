		...
		case kBinaryType:
			{
				int nByteCount = 1;
				int nPos;
				char* pszBuffer = new char[strlen(data.c_str()) + 1];
                strcpy(pszBuffer, data.c_str());
...
//	char* pszBuffer = _strdup(stringValue.c_str());
				char* pszBuffer = new char[strlen(stringValue.c_str()) + 1];
                strcpy(pszBuffer, stringValue.c_str());
	char* pszVal = NULL;
	uint32 iVal = 0;
...
{
	delete [] m_pRawData;   // !!! Fixed to prevent memmanager complaint
	m_pRawData = NULL;
	m_nLength = 0;
}
